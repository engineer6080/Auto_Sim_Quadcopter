...
class QuadSim():
    ...
    def __init__(self,quads,gravity=9.81,b=0.0245):
        ...
        env_bounds = 20.0  # 300 m / 300 m / 300 m
    ...
    def update(self, dt):
        ...
        for key in self.quads:
            ...
            self.pose[:3] = self.quads[key]['state'][0:3]
            self.pose[3:] = self.quads[key]['state'][6:9]
            self.v = self.quads[key]['state'][3:6]
            self.angular = self.quads[key]['state'][9:12]
            self.time += 1/50.
            
            for ii in range(3):
                if self.pose[ii] <= self.lower_bounds[ii]:
                    bounds = True
                elif self.pose[ii] > self.upper_bounds[ii]:
                    bounds = True
    ...
    def set_position(self,quad_name,position):
        ...
        self.pose[:3] = self.quads[quad_name]['state'][0:3]
        self.pose[3:] = self.quads[quad_name]['state'][6:9]
    ...
    def reset(self):
        self.quads['q1']['state'] = np.zeros(12)
        self.time = 0.
